        ...
        <div className="text-sm gap-1 flex flex-col " key={item.title}>
          ...
          {item.items.map((i) => {
            ...
            <Link
              key={i.label}
              href={i.href}
              className="flex gap-3 justify-center md:justify-start py-1 hover:bg-slate-50 rounded-sm shadow-sm text-gray-500 hover:text-gray-900 items-center no-underline px-3"
            >
              <Image src={i.icon} width={20} height={20} alt={i.label} />
              <p className="hidden md:block my-1">{i.label}</p>
            </Link>
          )
            }
          })}
